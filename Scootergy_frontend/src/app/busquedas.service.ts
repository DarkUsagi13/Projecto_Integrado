...
export class BusquedasService {
  ...
  buscarConexiones(userId: string, patinete: string, estacion: string, fecha_inicio: string, fecha_fin: string): Observable<any> {
    const params = new HttpParams().set('usuario', userId).set('finalizada', 'true').set('patinete', patinete).set('estacion', estacion).set('fecha_inicio', fecha_inicio).set('fecha_fin', fecha_fin);
